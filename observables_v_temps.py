...
df = pd.read_csv("temp_graph_machines.txt", sep="\t", names=names)
df2 = pd.read_csv("training_data_observables.txt", sep="\t", names=names)

#print(df)

#quant = 5

fig, axes = plt.subplots(4, 1, sharex='all', figsize=(2,8))
...
for i in range(4):
	...
	#axes[i].errorbar(df.Temperature, df[names[quant]], yerr=df[names[quant + 1]], fmt='o', ms='6')
